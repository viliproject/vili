...
export function setData(state, env, name, data) {
  return state.updateIn(["envs", env], new EnvRecord(), e =>
    e.mergeDeepIn(["keys", name], data)
  )
}
...
function initObjects(state, env, list, Model) {
  ...
  list.forEach(item => {
    ...
    state = state.setIn(
      ["envs", env, "keys", name, "object"],
      getModelInstance(Model, item)
    )
  })
  return state
}
...
export function changeObject(state, action, Model) {
  ...
  switch (eventType) {
    ...
    case "ADDED":
    case "MODIFIED":
      return setData(state, env, object.name || object.metadata.name, {
        object: getModelInstance(Model, object),
      })
    case "DELETED":
      return deleteData(state, {
        payload: { env, name: object.name || object.metadata.name },
      })
  }
  return state
}
